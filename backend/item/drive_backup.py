"""Google Drive backup/restore — supports both PostgreSQL and SQLite (see
backend.settings' DB_ENGINE toggle), dispatching on whichever is actually
configured.
...
Postgres backups are *data-only* SQL dumps (no CREATE TABLE/DDL) scoped to
the item app's own tables (see BACKUP_TABLES) — not Django's internal
auth/contenttype/session tables, which `migrate` repopulates on its own and
would otherwise collide on restore. SQLite backups are a full file-level
snapshot instead (see _create_backup_sqlite) — there's no separate dump
format/tool for SQLite, the whole DB already is one file.

A Postgres backup can only restore into a Postgres deployment, and a
SQLite backup only into a SQLite one — see create_backup's docstring for
why unifying the two formats isn't worth it. Restoring in 'strict' (the
default) or 'overwrite' mode assumes the target database's schema is
already up to date (i.e. `migrate` has run); 'merge' (SQLite only) instead
appends the backup's rows without touching what's already there.
"""
...
class ExistingDataError(DriveBackupError):
    ...
    items that already exist -- see _merge_backup_sqlite)."""
...
DUMP_TIMEOUT = 1800  # item_previewimage stores images as bytea/blob and runs several GB; the Pi is slow
...
def _create_backup_sqlite(progress_cb) -> dict:
    ...
    a straight file-level copy. A SQLite backup can only be restored into
    another SQLite deployment (see restore_backup).
    """
    import sqlite3
    from django.conf import settings as dj_settings
...
def restore_backup(file_id: str, mode: str = 'strict', progress_cb=None) -> None:
    ...
    `mode`:
      ...
      'merge'     -- keep existing data, append the backup's rows with
                  ...
                  source) pair -- see _merge_backup_sqlite. SQLite only;
...
def _merge_backup_sqlite(connection) -> dict:
    ...
    present, rather than ever risk losing local data):
      ...
      - Item: matched by (external_id, source) -- the same convention
        already used elsewhere in this app (see poll_twitter_updates.py's
        own known_ids) to mean "the same real post". A manually-registered
        item (source='manual', external_id a millisecond timestamp) has a
        small chance of coincidentally colliding with another manual item
        from the other device, but this mirrors how manual items are
        already deduplicated everywhere else in the app.
...
def _restore_backup_postgres(file_id: str, mode: str = 'strict', progress_cb=None) -> None:
    ...
    docstring for why). Surfacing a clear error is enough since this
    path is Postgres-only anyway (SQLite restores go through
    _restore_backup_sqlite, which does support it).
    """
    if mode == 'merge':
