...
export default function PreviewPane({open, onClose}){
  ...
  // load items with optional pagination; by default load first page only
  async function loadItems(url='/api/items/?page_size=50', replace=true){
    try{
  ...
  useEffect(()=>{
    if(!open) return
    loadItems('/api/items/?page_size=50', true)
  ...
  useEffect(()=>{
    function onItemPreviewUpdated(e){
      ...
      if(open){
        ...
        loadItems('/api/items/?page_size=50', true).then((loaded)=>{
