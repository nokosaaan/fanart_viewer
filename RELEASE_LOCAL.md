...
⚠️ **いいねは自動ポーリングの対象外**です（ログイン中アカウント自身のscreen_name解決が`twid`Cookieに依存しており、毎tick走らせるには不安定すぎるため）。いいねを取り込みたい場合はヘッダーメニューの「Twitterから画像取得」→「いいね」タブから都度手動で（ブックマークと同じUI、`twid`が未設定の場合はパネル内に警告表示）。`poller`は`SocialFetchQueueItem`の`kind='bookmark'`行しか触らないので、以前の自動いいね取得時代のキュー行が残っていても自動処理はされない。念のため削除したい場合:
...
**プレビュー未保存のアイテムも「既知」扱いされないよう修正**（`_known_twitter_ids`）。取得キューで画像選択待ちのまま放置されているアイテムがあると、それより古いブックマークの発見を同様にブロックしていた。

⚠️ **上記2つを直しても、通常のdiscoveryには構造的な限界が残ります**: 新しい方から辿って最初に「既知」のツイートに当たった時点で打ち切る仕組みのため、一時的な不具合等で一部だけ取り込みそびれた「抜け」（新しい既知ツイートと古い既知ツイートに挟まれた未取り込み分）は、通常運用では原理的に永久に発見できません。この抜けを回復したい場合は、ヘッダーメニューの「Twitterから画像取得」→「ブックマーク」タブ（「いいね」タブも同様）で**「完全スキャン(抜け漏れも探す)」にチェック**を入れて実行してください — 既知のツイートに当たっても打ち切らず、スキップして古い方まで探し続けます。ページ数上限を10〜20程度に上げて実行するのがおすすめ（1回で足りなければ複数回実行 — 前回の続きから再開します）。
...
**過去分をまとめて回復したい場合は「バックログを一括取得」を使う**（ヘッダーメニュー「Twitterから画像取得」→「ブックマーク」タブ内、通常の取得ボタン群の下）。現在のブックマークを新しい方から指定件数（デフォルト1万件、1ページ約20件）まで打ち切らずに読み切り、見つかった新規分をそのまま自動ポーリングと同じ待ち行列（`SocialFetchQueueItem`）に積む。実際の画像取得は行わず「発見して積むだけ」で、実取得は自動ポーリングが少しずつ進める。必ずバックグラウンドで実行（1万件だと数十分〜数時間かかる見込み）。

自動ポーリングが見つける本当に新しいブックマークは、Twitter自身のツイートID（作成順で単調増加）の降順で処理されるため、常にバックログより優先される — 同じ待ち行列に積んでも通常運用と競合しない（バックログの処理は、それより新しい分を処理し終えた後のtickでのみ少しずつ進む）。専用のカーソル（`bookmarks_full_scan_cursor`）を使うので、pollerの通常カーソルにも影響しない。
