...
デフォルトは単一キャラ画像のみで学習する（上の推奨構成の通り）。複数キャラが同時に写っている画像も学習に使いたい場合、`--include-multi-character`で有効化できる。仕組みは「まず単一キャラ画像だけで教師分類器を学習→人物検出のボックス数と確定キャラ数がちょうど一致する複数キャラ画像だけを対象に、教師モデルでどのボックスがどのキャラか推定→確信度(`--bootstrap-confidence`, デフォルト0.7)をクリアしたペアだけ疑似ラベルとして採用→単一キャラデータに追加して再学習」という自己学習(self-training)方式。
...
⚠️ **実データで検証済み（2026-09時点、単一キャラ3099枚＋複数キャラ1336件で実行）— holdout精度への効果は無し**（79.2% → 79.2%、差分0.0pt。train精度は88.5%→90.8%に上がるが過学習でholdoutには反映されない）。内訳:
- 複数キャラ1336件中798件（約60%）で人物検出のボックス数とキャラ数がクリーンに一致
- そのうち候補クロップ1794枚中820件（約46%）が確信度0.7をクリアして採用

割り当てメカニズム自体（どのボックスがどのキャラか）は個別に目視検証した範囲（最頻出ペア「シェリー＋ハンナ」378枚中の実サンプル）では正しく機能していたが、追加できる実質サンプル数（+820枚、既存の+26%程度）と、境界が不正確なクロップによる混入ノイズが相殺し合い、最終的なholdout精度は変わらなかった。**「サンプル数を稼ぎたい」という動機自体は妥当だが、現状のデータ規模ではこの手法で分類器の精度は上がらない** — 今後、複数キャラ画像・単一キャラ画像の絶対数がもっと増えた場合は再検証する価値がある。
